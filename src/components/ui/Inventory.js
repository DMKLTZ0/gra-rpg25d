...
export const Inventory = ({ show, onClose, inventory, equipment, useItem }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: '20px', paddingBottom: '16px', borderBottom: '2px solid rgba(255,255,255,0.2)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', fontSize: '12px' }}>
          {Object.entries(equipment).map(([slot, item]) => (
              <div key={slot} style={{ background: 'rgba(255,255,255,0.1)', padding: '8px', borderRadius: '8px', textAlign: 'center', color: '#fff' }}>
                  <div style={{ marginBottom: '4px' }}>{slot.charAt(0).toUpperCase() + slot.slice(1)}</div>
      ...
      {inventory.length === 0 ? (
        <div style={{ color: '#9ca3af', textAlign: 'center', padding: '20px' }}>Ekwipunek jest pusty</div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {inventory.map(item => (
            <div key={item.id} style={{
              ...
              <button onClick={() => useItem(item.id)} style={{
