...
export const useGameLogic = () => {
  ...
  const getTotalDefense = useCallback(() => {
    return Object.values(equipment).reduce((acc, item) => acc + (item?.defense || 0), 0);
  }, [equipment]);
  ...
  const getEnemyAt = useCallback((x, y) => {
  ...
  const attackEnemy = useCallback((x, y) => {
    setMapData(prev => {
  ...
  const createProjectile = useCallback((targetX, targetY) => {
    const newProjectile = {
      id: Date.now(),
      startX: playerPos.x,
      startY: playerPos.y,
  ...
  }, [getWeaponType, playerStats.attackRange, playerPos, direction, currentMapData.enemies, getEnemyAt, attackEnemy, createProjectile]);

  const usePortal = useCallback((x, y) => {
  ...
  const pickupItem = useCallback((x, y) => {
    ...
    if (item.type === 'gold') {
      ...
      displayMessage(`+${item.amount} złota!`);
       ...
       setInventory(prev => [...prev, {...item, id: `${item.type}_${Date.now()}`}]);
  ...
  const initiateMove = useCallback((dir) => {
    if (isMoving) return;
  ...
  }, [isMoving, playerPos, canMove, getEnemyAt, usePortal, pickupItem, currentMapData.portals, currentMapData.items]);
  ...
  const equipItem = useCallback((itemId) => {
    ...
    if (item.type === 'weapon') {
        ...
        setPlayerStats(prev => ({ ...prev, attackRange: item.range, damage: 25 + item.damage }));
  ...
  const handleButtonPress = useCallback((dir) => {
  ...
  const resetGame = useCallback(() => {
    setPlayerStats({
      hp: 100, maxHp: 100, level: 1, xp: 0, maxXp: 100,
      speed: 500, gold: 50, attackRange: 1, damage: 25
    });
    setPlayerPos({ x: 100, y: 50 });
    setCurrentMap('map_base1');
    setInventory([{ id: 'potion1', name: 'Mikstura zdrowia', type: 'potion', healing: 50, icon: '🧪' }]);
    setEquipment({ weapon: null, helmet: null, armor: null, pants: null, boots: null, shield: null });
  ...
  useEffect(() => {
    const moveEnemies = () => {
        ...
        setMapData(prev => {
            const newEnemies = prev[currentMap].enemies.map(enemy => {
                ...
                if (distance === 1) {
                    const baseDamage = { slime: 10, goblin: 15, skeleton: 20, orc: 25 }[enemy.type] || 10;
                ...
                if (distance > 1 && distance <= 10) {
                    let newX = enemy.x, newY = enemy.y, newDir = enemy.dir;
                    ...
                    if (canMove(newX, newY) && !getEnemyAt(newX, newY) && (newX !== playerPos.x || newY !== playerPos.y)) {
            ...
            return { ...prev, [currentMap]: { ...prev[currentMap], enemies: newEnemies } };
        });
    };
    ...
    const spawnEnemy = () => {
      ...
      const availableSpawns = currentMapData.spawnPoints.filter(p => Math.abs(p.x - playerPos.x) > 15 || Math.abs(p.y - playerPos.y) > 15);
      if (availableSpawns.length === 0) return;

      const spawnPoint = availableSpawns[Math.floor(Math.random() * availableSpawns.length)];
      const enemyTemplates = {
          map_base1: [{ type: 'slime', hp: 50, gold: 10, xp: 30 }, { type: 'goblin', hp: 80, gold: 25, xp: 50 }],
          map_cave1: [{ type: 'skeleton', hp: 120, gold: 40, xp: 70 }, { type: 'orc', hp: 150, gold: 60, xp: 100 }]
      };
      const template = enemyTemplates[currentMap][Math.floor(Math.random() * enemyTemplates[currentMap].length)];
      ...
      const newEnemy = {
        ...
        gold: template.gold, xp: template.xp
      };

      setMapData(prev => ({ ...prev, [currentMap]: { ...prev[currentMap], enemies: [...prev[currentMap].enemies, newEnemy] } }));
      setLastRespawn(now);
    };

    const interval = setInterval(() => { moveEnemies(); spawnEnemy(); }, 500);
    return () => clearInterval(interval);
  }, [playerPos, currentMap, currentMapData, canMove, getEnemyAt, getTotalDefense, lastRespawn]);
  ...
  return {
    playerPos, playerStats, equipment, inventory, projectiles, showInventory,
    cameraOffset, direction, isMoving, targetPos, animationProgress, showMessage,
    currentMap, currentMapData, TILE_SIZE, VISIBLE_TILES_X, VISIBLE_TILES_Y,
    setShowInventory, performAttack, useItem, handleButtonPress, handleButtonRelease,
    resetGame, getCurrentDisplayPosition, getTotalDefense, getWeaponType
  };
};
